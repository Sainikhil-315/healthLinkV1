...
api.interceptors.response.use(
  ...
  async error => {
    ...
    // Detailed debug logging for all API response errors
    try {
      console.error('API response error:', {
        message: error.message,
        url: originalRequest?.url || error.config?.url,
        method: originalRequest?.method || error.config?.method,
    ...
    // Network error / no response (server unreachable, CORS or network issue)
    if (!error.response) {
      error.userMessage =
        'Network error. Unable to reach backend at the configured API URL. Check server, firewall, and device network.';
      return Promise.reject(error);
    }
...
export const apiService = {
  ...
  getNearbyEmergencies: location =>
    api.get('/emergency/nearby/active', { params: location }),
  updateEmergencyStatus: (incidentId, status) =>
    api.put(`/emergency/${incidentId}/status`, { status }),
  cancelEmergency: (incidentId, reason) =>
    api.post(`/emergency/${incidentId}/cancel`, { reason }),
  resolveEmergency: (incidentId, outcome) =>
    api.post(`/emergency/${incidentId}/resolve`, { outcome }),
  acceptEmergencyRequest: incidentId =>
    api.post(`/emergency/${incidentId}/accept`),
  declineEmergencyRequest: (incidentId, reason) =>
    api.post(`/emergency/${incidentId}/decline`, { reason }),
  getEmergencyTracking: incidentId =>
    api.get(`/emergency/${incidentId}/tracking`),
  getEmergencyTimeline: incidentId =>
    api.get(`/emergency/${incidentId}/timeline`),
  rateResponder: (incidentId, rating) =>
    api.post(`/emergency/${incidentId}/rate`, rating),

  // Hospital
  registerHospital: data => api.post('/hospitals/register', data),
  getHospitalProfile: hospitalId =>
    api.get(`/hospitals/${hospitalId || 'profile'}`),
  ...
  confirmPatientArrival: incidentId =>
    api.post(`/hospitals/patients/${incidentId}/arrived`),
  getAvailableHospitals: location =>
  ...
  getAmbulanceProfile: ambulanceId =>
    api.get(`/ambulances/${ambulanceId || 'profile'}`),
  updateAmbulanceProfile: data => api.put('/ambulances/profile', data),
  updateAmbulanceLocation: location =>
    api.put('/ambulances/location', { location }),
  updateAmbulanceStatus: status => api.put('/ambulances/status', { status }),
  getAvailableAmbulances: location =>
    api.get('/ambulances/available/nearby', { params: location }),
  ...
  getVolunteerProfile: volunteerId =>
    api.get(`/volunteers/${volunteerId || 'profile'}`),
  updateVolunteerProfile: data => api.put('/volunteers/profile', data),
  updateVolunteerLocation: location =>
    api.put('/volunteers/location', { location }),
  updateVolunteerStatus: status => api.put('/volunteers/status', { status }),
  uploadCertificate: formData =>
    api.post('/volunteers/certificate', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    }),
  getCurrentMission: () => api.get('/volunteers/mission/current'),
  getMissionHistory: params =>
    api.get('/volunteers/mission/history', { params }),
  acceptMission: incidentId =>
    api.post(`/volunteers/mission/${incidentId}/accept`),
  arrivedAtScene: incidentId =>
    api.post(`/volunteers/mission/${incidentId}/arrived`),
  completeMission: (incidentId, notes) =>
    api.post(`/volunteers/mission/${incidentId}/complete`, { notes }),
  declineMission: (incidentId, reason) =>
    api.post(`/volunteers/mission/${incidentId}/decline`, { reason }),
  ...
  acceptDonationRequest: incidentId =>
    api.post(`/donors/request/${incidentId}/accept`),
  declineDonationRequest: (incidentId, reason) =>
    api.post(`/donors/request/${incidentId}/decline`, { reason }),
  completeDonation: incidentId =>
    api.post(`/donors/request/${incidentId}/complete`),
  getDonorStats: () => api.get('/donors/stats'),
  findCompatibleDonors: params =>
    api.get('/donors/find/compatible', { params }),
  ...
  verifyAmbulance: ambulanceId =>
    api.put(`/admin/ambulances/${ambulanceId}/verify`),
  getAllHospitals: params => api.get('/admin/hospitals', { params }),
  verifyHospital: hospitalId =>
    api.put(`/admin/hospitals/${hospitalId}/verify`),
  updateHospitalStatus: (hospitalId, data) =>
    api.put(`/admin/hospitals/${hospitalId}/status`, data),
  getAllVolunteers: params => api.get('/admin/volunteers', { params }),
  verifyVolunteer: volunteerId =>
    api.put(`/admin/volunteers/${volunteerId}/verify`),
  rejectVolunteer: (volunteerId, reason) =>
    api.put(`/admin/volunteers/${volunteerId}/reject`, { reason }),
  ...
  cancelIncident: (incidentId, reason) =>
    api.put(`/admin/incidents/${incidentId}/cancel`, { reason }),
